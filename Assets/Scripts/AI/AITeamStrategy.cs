...
namespace RacingManager.AI
{
    ...
    public class AITeamStrategy : MonoBehaviour
    {
        ...
        [Header("Sponsor Relations")]
        [Tooltip("Placeholder for sponsor satisfaction, from 0 (very unhappy) to 1 (very happy).")]
        [Range(0, 1)]
        public float sponsorSatisfaction = 0.5f;
        ...
        /// <summary>
        /// Updates the team's overall strategy based on various factors, like sponsor satisfaction.
        /// </summary>
        public void UpdateTeamStrategy()
        {
            // This logic replaces the purely random selection from A-001.
            // It now considers sponsor satisfaction to make a weighted choice.
            // Low satisfaction should push the AI towards more aggressive spending to meet goals.

            // Define weights for each state.
            float aggressiveWeight = 1.0f - sponsorSatisfaction; // High weight if satisfaction is low
            float balancedWeight = 0.5f; // Balanced is always a moderate option
            float saveMoneyWeight = sponsorSatisfaction * 0.75f; // Higher chance to save money if satisfaction is high

            // Total weight for normalization
            float totalWeight = aggressiveWeight + balancedWeight + saveMoneyWeight;

            // Get a random value within the total weight
            float random = Random.Range(0, totalWeight);

            // Determine the state based on the weighted random value
            if (random < aggressiveWeight)
            {
                CurrentState = TeamStrategyState.AggressiveDevelopment;
            }
            else if (random < aggressiveWeight + balancedWeight)
            {
                CurrentState = TeamStrategyState.Balanced;
            }
            else
            {
                CurrentState = TeamStrategyState.SaveMoney;
            }

            Debug.Log($"AI Team strategy updated to: {CurrentState} (Sponsor Satisfaction: {sponsorSatisfaction})");
        }
